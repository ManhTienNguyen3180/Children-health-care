...
public interface BlogRepository extends JpaRepository<Blog, Integer> {
        ...
        @Query(value = "INSERT INTO `childrencare_system`.`blog` (`category_blog_id`,`title`,`date`,`description`,`status`,`author`,`image`,`content`,`updateDate`) \n"
                        + //
        ...
        @Modifying
        @Transactional
        @Query(value = "update blog set `updateDate`= :update, `status`= :status where `blog_id`= :id", nativeQuery = true)
        void saveStatus(@Param("id") Integer id, @Param("update") String update, @Param("status") Integer status);

        @Modifying
        @Transactional
        @Query(value = "UPDATE blog SET `updateDate`= :update, `category_blog_id` = :category, `title` = :title, `date` = :date, `description` = :description, `status` = :status, `author` = :author, `image` = :image,  `content` = :content WHERE `blog_id` = :id", nativeQuery = true)
